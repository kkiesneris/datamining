...
for fRow in fraudCursor.execute(getFraudData):
    ...
    getPosTrans = """
            ...
            and retl_sic_cde = :3
            and retl_term_id = :4
            and retl_ky_rdfkey_id = :5
            and tran_datetime = to_date(:6,'YYYYMMDDHH24MISS')
    """
    #and trunc(tran_datetime,'DAY') + 1 > trunc(to_date(:6,'YYYYMMDDHH24MISS'),'DAY')
    #and trunc(tran_datetime,'DAY') - 1 < trunc(to_date(:7,'YYYYMMDDHH24MISS'),'DAY')

    tranCursor.execute(getPosTrans,(fPAN,fAmount,fMCC,fTerminalId,fMerchantId,fTranDate))
    ...
    for tRow in tRows:
        ...
        if len(tRows)==1:
            ...
            updatePosTrans = """
                update pos_trans set arn = :1 , fraud = '1100' where crd_card_crd_num = :2 and amt_1 = :3 and tran_dat = :4 and tran_tim = :5
            """
            updateCursor.execute(updatePosTrans,(fARN,tPAN,tAmount,tTranDate,tTranTime))
            updateCursor.execute("update mc_acq set fraud = '1100' where arn = :1",(fARN,))
            connection.commit()
