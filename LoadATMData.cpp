...
string getInsertStatement(vector<field> &fieldList) {
    ...
    string sql = "INSERT INTO ATM_TRANS (" + parameterNames + ") VALUES (" + parameterValues + ")";
    return sql;
}

void writeToLogFile(string text) {
    ofstream oFile;
    oFile.open("load_data_atm_log.txt", ios::app);
    if (oFile.is_open()) {
        oFile << text << endl;
        oFile.close();
    } else {
...
static const unsigned char arrayEbcdicBin[256]=
{
    0x20,0x01,0x02,0x03,0x04,0x09,0x06,0x7f,
    0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F,
    0x10,0x11,0x12,0x13,0x14,0x0A,0x08,0x17,
    0x18,0x19,0x1A,0x1B,0x1C,0x1D,0x1E,0x1F,
    0x20,0x21,0x1C,0x23,0x24,0x0A,0x17,0x1B,
    0x28,0x29,0x2A,0x2B,0x2C,0x05,0x06,0x07,
    0x30,0x31,0x16,0x0D,0x34,0x35,0x36,0x04,
    0x38,0x09,0x0C,0x3B,0x14,0x15,0x3E,0x1A,
...
int main() {
    ...
    try {
        ...
        if (getFieldList(fieldList,"cfg/ATM.xml")) {
        ...
        Connection *con = env->createConnection(user, pass, conString);
        ...
        string line;
        size_t bufferSize = 4096*4;
        char *buffer = new char[bufferSize];
        string delimiter = "\x0A";
        string bucket;
    
        string filename = "atm.txt";
        ifstream dataFile(filename, std::ios::binary);
        ...
        while(dataFile) {
            ...
            size_t count = dataFile.gcount();

            if (!count) 
                break;

            for (int i= 0;i<count;i++){
                buffer[i]= arrayEbcdicBin[buffer[i]];
            }

            bucket+= string((const char*)buffer,count);
            while (true) {
                size_t pos = bucket.find(delimiter);
                if (pos==string::npos) break;
                string line = bucket.substr(0,pos);
                if (delimiter == "\x0A" && !line.empty() && line[pos-1] == 0x0D) {
                    line.erase(pos - 1);
                }
                bucket.erase(0,pos+delimiter.size());

                rowsProcessed++;
                if (rowsToSkip > rowsProcessed) {
                    //skip
                    ...
                    ofstream errorFile;
                    errorFile.open("skipped_atm_lines.txt", ios::app);
                    if (errorFile.is_open()) {
                        errorFile << line <<endl;
                        errorFile.close();
                    } else {
                        cout << "Unable to open skipped_atm_lines.txt file" << endl;
                    }
                } else {
                    bindNr = 1;
