...
getFraudData = """
    select degraded_pan, tran_date, to_char(round(amount/0.0395)) as amnt, term_id, merchant_id, merchant_name, merchant_city, country, mcc, arn
    from visa_iss
    where fraud is null
...
for fRow in fraudCursor.execute(getFraudData):
    ...
    getPosTrans = """
            ...
            and (amt_1 * 0.95) < to_number(:4)
            and (amt_1 * 1.05) > to_number(:5)
            and term_cntry_cde = :6
            and retl_sic_cde = :7
    """
    tranCursor.execute(getPosTrans,(fPAN,fTranDate,fTranDate,fAmount,fAmount,fCountry,fMCC))
    ...
    for tRow in tRows:
        ...
        if len(tRows)==1:
            ...
            updateCursor.execute("update visa_iss set fraud = '1101' where arn = :1",(fARN,))
            connection.commit()
